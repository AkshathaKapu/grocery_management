...
ObservableList<PurchaseCls> purlist=FXCollections.observableArrayList();
    ...
    String t1=String.valueOf(t);
        ...
        int newstock=0;
        newstock=stock-(Integer.parseInt(q));
        if(newstock>=0)
        {
...
//    DefaultTableModel model=(DefaultTableModel) jTable1.getModel();
    ...
    totalbillbox.setText(String.valueOf(totalbill));
        ...
        {
             erralert.setTitle("Stock Is not enough");
            String s = "Product "+name+" has left only "+stock+" item\n Check your Required Quantity";
    ...
    try 
        {
           ...
           try
           {
               String qry="select * from sell order by sellid desc";
               PreparedStatement st=(PreparedStatement) conn.prepareStatement(qry);
                ResultSet result=st.executeQuery();
                 if(result.next())
                 {
                    
                     int i=result.getInt("sellid");
                     i+=1;
  ...
  void addselldetail()
    {
        try 
        {
           ...
           try
           {
                ...
                for(PurchaseCls yd : data) 
                  {
                    ...
                    String qry="insert into selldetail (sellid,custname,productname,price,qty,totalamt,billdate)values(?,?,?,?,?,?,?)";
                    PreparedStatement st=(PreparedStatement) conn.prepareStatement(qry);
                    String q=puridbox.getText();
                    st.setString(1,q);

                    st.setString(2,customername.getText());
               ...
               {
                   erralert.setTitle("Sell Detail ");
                    String s = "Empty data" + "Sell some items first";
                    erralert.setContentText(s);
                    erralert.showAndWait();
           ...
           {
               erralert.setTitle("Sell Detail");
            String s = "Sell Detail Error in Query " + e;
            erralert.setContentText(s);
            erralert.showAndWait();
           }
  ...
  int fetch_pro_stock(String name)
{
    ...
    void addsell()
    {
        try 
        {
           ...
           try
           {
               String qry="insert into sell values(?,?,?,?,?,?)";
               PreparedStatement st=(PreparedStatement) conn.prepareStatement(qry);
                ...
                st.setString(3, d3);
                 ...
                 if(result>0)
                 {
                    infoalert.setTitle("Sell");
                    infoalert.setHeaderText("Added  ");
                    String s ="Sell details Added Successfully ";
                    infoalert.setContentText(s);
                    infoalert.show();
